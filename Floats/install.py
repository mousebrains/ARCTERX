#! /usr/bin/env python3
#
# Install a service for scrapping the UW/APL Seaglider positions
#
...
parser.add_argument("--service", type=str, default="Zoli2DB.service", help="Service file")
parser.add_argument("--timer", type=str, help="Timer file")
...
if args.timer:
    ...
    s = subprocess.run((args.sudo, args.systemctl, "--no-pager", "status", 
                        args.service, args.timer), shell=False, check=False)
else:
    print(f"Status {args.service}")
    s = subprocess.run((args.sudo, args.systemctl, "--no-pager", "status", 
                        args.service), shell=False, check=False)
